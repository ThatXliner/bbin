...
_ansi_prefix: str = "\033["  # Sh can use this. Although \x1b is preferred


def emoji_check(message: str) -> str:
    """Encode and decode a string into a compatible string"""
    return message.encode().decode(_sys.getfilesystemencoding(), "ignore")
...
class Interface:
    ...
    def info(self, msg: str, *, err: bool = False, shutup: bool = False) -> None:
        ...
        click.echo(
            emoji_check("%sINFO: %s%s%s" % (self.BLUE, self.YELLOW, msg, self.RESET)),
    ...
    def error(self, ctx: click.Context, msg: str, errorcode: int = 1) -> NoReturn:
        """Raise an error"""
        ctx.fail(msg)
        ctx.exit(errorcode)
    ...
    def success(self, msg: str = "Success!", *, err: bool = False) -> None:
        ...
        click.echo(
            emoji_check("%s%s%s" % (self.GREEN, msg, self.RESET)),
